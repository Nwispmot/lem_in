...
void	ft_insert_links(t_room **all_rooms, t_room *room, int neighbour)
{
	...
	if (room->links == NULL)
	{
		...
		//room->links[0] = (t_room*)malloc(sizeof(t_room));
		//room->links[0]->name = NULL;
		//room->links[0]->empty_fl = 0;
		room->links[1] = NULL;
        room->links[0] = all_rooms[neighbour];
        room->links[0]->empty_fl = 1;
        //ft_bzero(room->links, sizeof(t_room));
//        while (j < 10)
//        {
//            room->links[j] = (t_room*)malloc(sizeof(t_room));
//            room->links[j]->empty_fl = 0;
////            room->links[j] = NULL;
//////            room->links[j]->name = ft_strnew(0);
//////            room->links[j]->empty_fl = 0;
//            j++;
//        }
//		while (j < 10)
...
char	**ft_make_matrix(char **map, t_room **room, t_lem *lem)
{
	...
	while (room[j] != NULL)
	{
		...
		while (room[j]->links[i] != NULL)
		{
			if (room[j]->links[i]->empty_fl == 1)
		        ft_printf("name: %s\n", room[j]->links[i]->name);
			i++;
		}
		ft_printf("\n");
